...
export default function MobileHeader() {
  ...
  return (
    <>
      <div className="lg:hidden w-full h-16 bg-[var(--bg-main)]/80 backdrop-blur-xl border-b border-[var(--border)] px-5 md:px-8 flex items-center justify-between sticky top-0 z-50 transition-all duration-300">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl overflow-hidden border border-[var(--border)] shadow-lg shadow-[var(--accent)]/20 rotate-3">
            <img src="/avatar.png" alt="Avatar" className="w-full h-full object-cover object-top" />
          </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-[85%] max-w-[320px] bg-[var(--bg-main)]/95 backdrop-blur-2xl z-[70] border-l border-[var(--border)] overflow-y-auto transition-transform duration-500 ease-out lg:hidden flex flex-col pt-8 pb-8 shadow-2xl ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="px-8 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl overflow-hidden border border-[var(--border)] shadow-lg shadow-[var(--accent)]/10">
              <img src="/avatar.png" alt="Avatar" className="w-full h-full object-cover object-top" />
            </div>
            <div className="flex flex-col -space-y-1">
              <span className="text-xs font-black text-[var(--text-main)] uppercase tracking-tight">{tStatus("name")}</span>
              <span className="text-[8px] font-extrabold text-[var(--text-secondary)] uppercase tracking-[0.2em] opacity-60">Portfolio</span>
            </div>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setIsOpen(false)}
                className={`group flex items-center gap-4 px-4 py-3 rounded-2xl text-[12px] font-extrabold transition-all duration-300 relative ${isActive
        ...
        <div className="mt-auto px-8 py-6 border-t border-[var(--border)]/20 bg-[var(--bg-secondary)]/30 flex flex-col gap-6">
          ...
          <div className="text-center space-y-1">
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--text-main)] opacity-40">
              &copy; {new Date().getFullYear()} {tFooter("copyright")}
            </p>
            <p className="text-[8px] font-bold text-[var(--text-secondary)] opacity-50 uppercase tracking-widest leading-relaxed">
              {tFooter("right")}
            </p>
          </div>
        </div>
